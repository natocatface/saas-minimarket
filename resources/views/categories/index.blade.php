...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h2 class="text-xl font-extrabold text-slate-800">Categorías</h2>
            <p class="text-sm text-slate-500">Organiza tus productos por categorías</p>
        </div>
        <a href="{{ route('categorias.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-brand-500 to-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow hover:from-brand-600 hover:to-emerald-700">
    ...
    <div class="rounded-2xl bg-white border border-slate-100 shadow-sm overflow-hidden">
        <form method="GET" class="p-4 border-b border-slate-100">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Buscar categoría..."
                class="w-full sm:w-72 rounded-lg border border-slate-200 px-4 py-2 text-sm focus:border-brand-500 focus:ring-2 focus:ring-brand-100 outline-none">
        </form>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($categories as $cat)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-5 py-3 text-center text-slate-600">{{ $cat->products_count }}</td>
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('categorias.edit', $cat) }}" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-brand-600" title="Editar">
